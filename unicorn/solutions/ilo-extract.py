...
ENTRY   = 0x7F37a0  # Where we start
STOP    = 0x7F39ec  # We want to get there
...
def fix_decomp_args(mu, address, size, user_data):
    # R0 = 1st arg = 0x440 + 0x440 + 4 (from 0x7f3930, offset 0x880 in file)
    # Would be offset where decompression should start
    mu.reg_write(UC_ARM_REG_R0, 0x440 + 0x440 + 4)
    # R1 = 2nd arg = 0x440+0x440, length of compressed data, need to dereference pointer
    mu.reg_write(UC_ARM_REG_R1, struct.unpack("<I", mu.mem_read(0x440 + 0x440, 4))[0])
...
try:
    ...
    # constants we know from code
    mu.mem_write(0xf9fffffc, struct.pack("<I", 0x334f4c69))
    ...
    mu.hook_add(UC_HOOK_CODE, fix_decomp_args, begin=ADDRESS + 0x7f39e4, end=ADDRESS + 0x7f39e4)
    ...
    flagsize = mu.reg_read(UC_ARM_REG_R3)
